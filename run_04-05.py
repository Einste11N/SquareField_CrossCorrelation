...
l1_list = tc.hstack([ 10**tc.linspace(-4, 0, 5)[:-1], 10**tc.linspace(0, 1, 6)[:-1], tc.linspace(10, 300, 59), (10**tc.linspace(np.log10(300), np.log10(600), 13))[1:] ])

params = []
for zindex in range(Nz):
    for l in l_list:
        for pz_index in range(len(pz_list)):
            for l1 in l1_list:
